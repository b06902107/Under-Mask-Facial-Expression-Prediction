...
if __name__ == "__main__":
    ...
    with open(dataset) as csv_file:
        ...
        for i, row in tqdm(enumerate(csv_reader)):
            ...
            '''
    #print(data)
    with open('fer2013maskSave.csv', 'w', newline = '') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['emotion', 'pixels', 'Usage'])
        for i, img, usage in tqdm(inputs):
            try:
                print('fuck')
                mask_img = overlayMask(img, mask_img_name, mask_csv)
                mask_img = np.reshape(mask_img, (1, -1))
                tmp = " ".join(str(int(i)) for i in mask_img[0])
                print(i, tmp, usage)
                writer.writerow([i, tmp, usage])
            except:
                continue
